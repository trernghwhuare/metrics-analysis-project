...
def process_network(network_name, data_dir="gt/params", max_count=300, no_offscreen=False):
    ...
    try:
        ...
        def update_state():
            nonlocal count, frame_limits, all_x_positions, all_y_positions, vertex_index_map, edge_index_map
            # Reset properties
            ...
            if count == 0 and active_count == 0:
                ...
                num_initial_infections = min(30, len(nodes), len(input_nodes))  # Infect at most 30 nodes
                ...
                try:
                    ...
                    for v in g.vertices():
                        if curr_state[v] == I:  # If vertex is active (infected)
                            ...
                            acitve_sizes.append(dynamic_node_size * 2.0)
                    if active_x_pos:
                    ...
                    for v in g.vertices():
                        if curr_state[v] == S:  # If vertex is inactive
                            ...
                            inactive_sizes.append(dynamic_node_size * 0.5)
                    if inactive_x_pos:
                    ...
                    for v in g.vertices():
                        if curr_state[v] == R:
                            ...
                            refrac_sizes.append(dynamic_node_size * 1.2)
                    if refractory_x_pos:
        ...
        if offscreen:
            ...
            print(f"ffmpeg -framerate 10 -pattern_type glob -i '{network_frames_dir}/frame_*.png' -vf 'scale=1920:1080' -c:v libx264 -pix_fmt yuv420p {network_name}_combined.mp4")
            print("\nAlternatively, you can create an animated GIF using ImageMagick:")
            print(f"convert -delay 5 {network_frames_dir}/*.png {network_name}_combined.gif")
            print("\nFor large networks, create optimized GIF with reduced resolution:")
            print(f"convert -delay 5 -loop 0 {network_frames_dir}/*.png -scale 1200x1200 -coalesce -fuzz 5% -layers Optimize {network_name}_combined.gif")
            print("\nThis will create an animated GIF from the saved frames.")
            print("\nTo run without saving frames, use: python gt_state_dynamic.py no_offscreen")
        else:
            print(f"\nRunning {network_name} without saving frames.")
            print("To save frames, run without arguments: python gt_state_dynamic.py")
