...
def main():
    ...
    p.add_argument("--use-undirected", action="store_true", help="Use undirected graph for eigenvector-based metrics (eigenvector, katz, eigentrust, trust_transitivity)")
    args = p.parse_args()
    ...
    compute_and_save_metrics(G, out_dir=args.out, prefix=args.prefix, normalize=args.normalize, nthreads=args.threads, save_files=True, use_undirected_for_eigenvector=args.use_undirected)
