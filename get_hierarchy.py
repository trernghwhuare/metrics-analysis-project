...
def visualize_network(nml_net_file, V_intra, V_inter, R_intra, R_inter, L_intra, L_inter, base_name):
    output_dir = f"original/{base_name}"
    ...
    def animate_hierarchy(state, output_gif, frames=5, sweeps_per_frame=50, **draw_kwargs):
        ...
        for i in range(frames):
            ...
            frame_file = f"original/{base_name}/hierarchy_frame_{i:03d}.png"
    ...
    for i in range(4):
        ...
        state_ndc_full.draw(
            ...
            output=f"original/{base_name}/{base_name}_ndc_hierarchy_{i}.png",
            empty_branches=False,
        )
    animate_hierarchy(state_ndc_full, output_gif=f"original/{base_name}/{base_name}_ndc_hierarchy.gif",
    ...
    for i in range(4):
        ...
        state_dc_full.draw(
            bg_color=[0.98, 0.98, 0.98, 1],
            output=f"original/{base_name}/{base_name}_dc_hierarchy_{i}.png",
            empty_branches=False,
        )
    animate_hierarchy(state_dc_full, output_gif=f"original/{base_name}/{base_name}_dc_hierarchy.gif",
                    frames=5, sweeps_per_frame=50, bg_color=[0.98, 0.98, 0.98, 1])
    ...
    for i, mode in enumerate(modes):
        ...
        state_hierarchy.draw(vertex_shape="pie", vertex_pie_fractions=pv,
                             bg_color=[0.98, 0.98, 0.98, 1],output=f"original/{base_name}/{base_name}_hierarchy_{i}.png" ,empty_branches=False)
    animate_hierarchy(state_hierarchy,output_gif=f"original/{base_name}/{base_name}_hierarchy.gif",frames=5,sweeps_per_frame=50,bg_color=[0.98, 0.98, 0.98, 1])
    ...
    def animate_ghcp(state, tpos, shape, cts, eprop_color, output_gif,frames=5, sweeps_per_frame=50, vertex_fill_color=None, 
        ...
        for i in range(frames):
            ...
            frame_file = f"original/{base_name}/ghcp_frame_{i:03d}.png"
            gt.graph_draw(
                G, pos=tpos,
                vertex_shape=shape,
    ...
    gt.graph_draw(G, pos=G.own_property(tpos), 
                ...
                bg_color=[0.98, 0.98, 0.98, 1],output=f"original/{base_name}/{base_name}_ghcp.png")
    for i in range(100):
    ...
    state.draw(edge_color=edge_weight.copy("double"), ecmap=matplotlib.cm.plasma,
                    ...
                    edge_gradient=[], bg_color=[0.98, 0.98, 0.98, 1],output=f"original/{base_name}/{base_name}_ghcp_wsbm.png")
    animate_ghcp(state, tpos, shape, cts, eprop_color,output_gif=f"original/{base_name}/{base_name}_ghcp.gif",frames=5, sweeps_per_frame=50)
